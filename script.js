...
function showToast(message, type = "info") {
  appState._lastToastTick = Date.now();
...
function trainingDurationMonthsByTier(tier) {
  const t = String(tier || "").toLowerCase();
  if (t === "frontline") return 3;
  if (t === "intermediate") return 12;
  return 24; // advanced
}

function enforceTierHorizonCohortConstraints(opts = {}) {
  const { toastOnClamp = false } = opts;
  ...
  if (!tierEl || !horizonEl || !cohortsEl) return;

  const tier = String(tierEl.value || "frontline");
  let horizonYears = Number(horizonEl.value || 1);
  if (!isFinite(horizonYears) || horizonYears < 1) horizonYears = 1;

  const dur = trainingDurationMonthsByTier(tier);
  let horizonMonths = Math.round(horizonYears * 12);

  // Advanced: enforce minimum 2 years
  if (tier === "advanced" && horizonMonths < 24) {
    horizonMonths = 24;
    horizonYears = 2;
    horizonEl.value = String(horizonYears);
    if (toastOnClamp) showToast("Advanced tier requires at least a 2-year planning horizon. Horizon adjusted to 2 years.", "warning");
  }

  const maxCohorts = Math.max(1, Math.floor(horizonMonths / dur));
  cohortsEl.max = String(maxCohorts);

  let cohorts = Number(cohortsEl.value || 1);
  if (!isFinite(cohorts) || cohorts < 1) cohorts = 1;
  if (cohorts > maxCohorts) {
    cohortsEl.value = String(maxCohorts);
    if (toastOnClamp) {
      showToast(`Number of cohorts adjusted to ${maxCohorts} to fit the ${horizonYears}-year horizon and ${dur}-month training duration.`, "warning");
    }
  }

  // Add/update a small hint below cohorts input
  const hintId = "cohortsMaxHint";
  let hint = document.getElementById(hintId);
  if (!hint) {
    hint = document.createElement("div");
    hint.id = hintId;
    hint.className = "hint";
    cohortsEl.parentNode && cohortsEl.parentNode.appendChild(hint);
  }
  hint.textContent = `Maximum cohorts allowed for this tier and horizon: ${maxCohorts}.`;

  // Recompute dependent outputs quickly if a scenario exists
  try {
    ...
    // Don't spam; only show if explicitly requested
  }
}

function updateCapacityFeasibilityUIFromConfig(config) {
  const statusEl = document.getElementById("capacity-status");
  const mentorsPerCohortEl = document.getElementById("capacity-mentors-per-cohort");
  const mentorsNationalEl = document.getElementById("capacity-total-mentors-required");
  const mentorsAvailEl = document.getElementById("capacity-available-mentors");
  const shortfallEl = document.getElementById("capacity-mentor-shortfall");
  const titleEl = document.getElementById("capacityCostsTitle");

  if (!statusEl && !mentorsPerCohortEl && !mentorsNationalEl && !mentorsAvailEl && !shortfallEl) return;

  const cap = computeCapacity(config || {});
  if (mentorsPerCohortEl) mentorsPerCohortEl.textContent = formatNumber(cap.mentorsPerCohort || 0, 0);
  if (mentorsNationalEl) mentorsNationalEl.textContent = formatNumber(cap.totalMentorsRequired || 0, 0);
  if (mentorsAvailEl) mentorsAvailEl.textContent = formatNumber(config.availableMentorsNational ?? 0, 0);
  if (shortfallEl) shortfallEl.textContent = formatNumber(cap.mentorShortfall || 0, 0);
  if (statusEl) statusEl.textContent = cap.status || "-";

  if (titleEl) {
    titleEl.classList.remove("is-ok","is-warn","is-bad");
    if (cap.mentorShortfall <= 0) titleEl.classList.add("is-ok");
    else titleEl.classList.add("is-warn");
  }
}

function generateRecommendedScenarioPool() {
  // Recommended scenarios are generated from the model logic, independent of user-saved scenarios.
  const tiers = ["frontline", "intermediate", "advanced"];
  const careers = ["certificate", "uniqual", "career_path"];
  const mentorships = ["low", "medium", "high"];
  const deliveries = ["blended", "inperson", "online"];
  const traineesGrid = [25, 35, 50];

  const pool = [];
  const base = getConfigFromForm();

  tiers.forEach((tier) => {
    const dur = trainingDurationMonthsByTier(tier);
    // Use 1 year for frontline/intermediate; 2 years for advanced
    const horizonYears = tier === "advanced" ? 2 : 1;
    const horizonMonths = horizonYears * 12;
    const maxCohorts = Math.max(1, Math.floor(horizonMonths / dur));
    const cohortsGrid = tier === "frontline" ? [2, 3, Math.min(4, maxCohorts)].filter((x) => x>=1 && x<=maxCohorts)
                    : tier === "intermediate" ? [1]
                    : [1];
    ...
    careers.forEach((career) => {
      mentorships.forEach((mentorship) => {
        deliveries.forEach((delivery) => {
          traineesGrid.forEach((trainees) => {
            cohortsGrid.forEach((cohorts) => {
              const cfg = {
                ...base,
                tier,
                career,
              ...
              try {
                ...
                s.shortlisted = false;
                s.capacity = computeCapacity(cfg);
                pool.push(s);
              } catch (e) {}
            });
          });
...
function refreshRecommendedScenarios() {
  // Seed based on cost slider + capacity + currency, to avoid regenerating unnecessarily
  const seed = JSON.stringify({
    currency: appState.currency,
    usdRate: appState.usdRate,
    costs: appState.epiSettings?.tiers || {},
    general: appState.epiSettings?.general || {},
    advMentors: appState.epiSettings?.general?.availableMentorsNational,
    advSites: appState.epiSettings?.general?.availableTrainingSites
  });
  if (appState.recommendedScenarioSeed === seed && appState.recommendedScenarios?.length) return;

  appState.recommendedScenarioSeed = seed;
  appState.recommendedScenarios = generateRecommendedScenarioPool();
}
...
function computeTopScenarioItems(metric, feasibleOnly, selectedOnly, limit) {
  const candidates = (appState.recommendedScenarios || []).map((s, idx) => ({ s, idx }))
...
function renderTopScenariosTable(prefix, items) {
  ...
  items.forEach((item, rankIdx) => {
    const s = item.s;
    const cap = item.cap || s.capacity || computeCapacity(s.config || {});
    const idx = item.idx;
    ...
    tr.innerHTML = `
      ...
      <td>
        <div class="row-actions">
          ...
          <button type="button" class="btn-link btn-link-small" data-top5-action="save" data-sidx="${idx}">Save</button>
  ...
  if (!isCopilot) {
    body.querySelectorAll("[data-top5-action='view']").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const s = (appState.recommendedScenarios || [])[idx];
        if (s) openSnapshotModal(s);
      });
    });

    body.querySelectorAll("[data-top5-action='save']").forEach((btn) => {
      btn.addEventListener("click", () => {
        const idx = Number(btn.getAttribute("data-sidx"));
        const s = (appState.recommendedScenarios || [])[idx];
        if (!s) return;
        appState.savedScenarios.push(cloneScenario(s));
        refreshSavedScenariosTable();
        refreshSensitivityTables();
        showToast("Recommended scenario saved to Saved scenarios.", "success");
      });
    });
    ...
    body.querySelectorAll("[data-top5-action='shortlist']").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        const s = (appState.recommendedScenarios || [])[idx];
        if (!s) return;
        s.shortlisted = !isScenarioShortlisted(s);
        refreshTopScenariosPanel("top5");
        showToast(s.shortlisted ? "Recommended scenario shortlisted." : "Recommendation removed from shortlist.", "success");
      });
    });
  }
}
...
function exportTop5OnlyPdf() {
  ...
  const rows = topScenarios.map((s, i) => {
    ...
    return [
      String(i + 1),
      getScenarioDisplayName(s) || `Scenario ${i + 1}`,
      formatNumber(s.endorseRate, 1) + "%",
...
function initSensitivityContractControls() {
  ...
  const pdfBtn = getElByIdCandidates(["downloadSensitivityPDF", "download-sensitivity-pdf", "downloadSensitivityPdf", "export-sensitivity-benefits-pdf"]);
...
function refreshAllOutputs(scenario) {
  ...
  updateMainOutputs(scenario);
  updateCharts(scenario);
  updateCostingOutputs(scenario);
  updateNationalOutputs(scenario);
  refreshSensitivityTables();

  try {
    const cfg = scenario && scenario.config ? scenario.config : getConfigFromForm();
    updateCapacityFeasibilityUIFromConfig(cfg);
  } catch (e) {}
...
function initEventHandlers() {
  ...
  if (updateBtn) {
    ...
    try {
      enforceTierHorizonCohortConstraints({ toastOnClamp: true });
      ...
      showToast("Could not compute results from the current configuration. Please check inputs and try again.", "error");
    }
  });
  }
  ...
  // Enforce tier–duration–horizon logic (immediate validation + clamping)
  const tierEl = document.getElementById("program-tier");
  const horizonEl = document.getElementById("planning-horizon-config");
  const cohortsEl = document.getElementById("cohorts");
  if (tierEl) tierEl.addEventListener("change", () => enforceTierHorizonCohortConstraints({ toastOnClamp: true }));
  if (horizonEl) horizonEl.addEventListener("input", () => enforceTierHorizonCohortConstraints({ toastOnClamp: true }));
  if (cohortsEl) cohortsEl.addEventListener("input", () => enforceTierHorizonCohortConstraints({ toastOnClamp: true }));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  initEventHandlers();
  enforceTierHorizonCohortConstraints({ toastOnClamp: false });
  try { refreshRecommendedScenarios(); refreshTopScenariosPanel('top5'); refreshTopScenariosPanel('top5-copilot'); } catch (e) {}
  // Fallback: ensure every button click surfaces a toast if the action didn't produce one.
  document.addEventListener("click", (ev) => {
    const btn = ev.target && ev.target.closest ? ev.target.closest("button") : null;
    if (!btn) return;
    const msg = btn.getAttribute("data-toast-message");
    if (!msg) return;
    const now = Date.now();
    window.setTimeout(() => {
      const last = appState._lastToastTick || 0;
      if (Date.now() - last < 250) return; // an explicit toast just fired
